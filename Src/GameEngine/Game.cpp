...
nts::Game::Game(my::ItemLocker<std::vector<std::shared_ptr<is::IEntity>>> &entities, my::ItemLocker<my::ThreadPool> &eventManager, irr::core::vector2di mapSize, bool splitScreen) : AManageIrrlicht(entities, eventManager), _mapSize(mapSize), _splitScreen(splitScreen)
{
...
irr::scene::ISceneNode *nts::Game::getNode(const std::shared_ptr<is::IEntity> &entity)
{
	return _listObj[entity].obj;
}

float &nts::Game::getNodeSize(const std::shared_ptr<is::IEntity> &entity)
{
	return _listObj[entity].size;
}

bool nts::Game::addEntity(std::shared_ptr<is::IEntity> &entity, irr::scene::ISceneNode *obj, float size)
{
